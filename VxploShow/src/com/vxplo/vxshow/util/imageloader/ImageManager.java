package com.vxplo.vxshow.util.imageloader;
...
public class ImageManager {
	...
	private static VxploApplication myapp;
	...
	public static ImageManager from(Context context) {
		...
		if (myapp == null) {
			myapp = (VxploApplication) context.getApplicationContext();
		}
...
//					System.out.println("dddddsss"+name);
					if (url.toLowerCase().contains("sdcard")||url.toLowerCase().contains("storage")) {
						...
						if(bitmap == null) {
							...
							if(bitmap != null) {
								if (imageRef.width != 0 && imageRef.height != 0) {
									//if (mDiskCache.get(url + imageRef.width
