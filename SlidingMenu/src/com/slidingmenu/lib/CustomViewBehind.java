...
public class CustomViewBehind extends ViewGroup {
	...
	private Drawable mloShadowDrawable;
	private Drawable mSecondaryShadowDrawable;
	private int mShadowWidth;
	private int mloShadowWidth;
	...
	public void setMloShadowDrawable(Drawable shadow) {
		mloShadowDrawable = shadow;
		invalidate();
	}
	...
	public void setMloShadowWidth(int width) {
		mloShadowWidth = width;
		invalidate();
	}
	...
	public void drawLeftOuterShadow(View content, Canvas canvas) {
		if (mloShadowDrawable == null || mloShadowWidth <= 0) return;
		int left = 0;
		if (mMode == SlidingMenu.LEFT) {
			left = content.getLeft() - mloShadowWidth;
		} else if (mMode == SlidingMenu.RIGHT) {
			left = content.getRight();
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			if (mSecondaryShadowDrawable != null) {
				left = content.getRight();
				mSecondaryShadowDrawable.setBounds(left, 0, left + mloShadowWidth, getHeight());
				mSecondaryShadowDrawable.draw(canvas);
			}
			left = content.getLeft() - mloShadowWidth;
		}
		mloShadowDrawable.setBounds(left, 0, left + mloShadowWidth, getHeight());
		mloShadowDrawable.draw(canvas);
	}
